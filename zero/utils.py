...
def aug(x, x_bar, ratio):
    ...
    # 获取x的第一个维度的大小
    sz = x.shape[0]
    # 从0到sz-1的整数中随机选择int(sz * ratio)个索引
    idx = np.random.choice(sz, int(sz * ratio))
    # 将ret_x中对应idx位置的值替换为x_bar中对应idx位置的值
    ret_x[idx] = x_bar[idx]
    return ret_x
...
def predict_dataset_softmax(predict_loader, model, device, train_num, threshold=0.95):
    ...
    model.eval()
    loss_outs = torch.zeros(train_num).to(device)   # 初始化损失输出数组
    crit = nn.CrossEntropyLoss(reduction='none')    # 定义交叉熵损失函数
    # 存储模型预测结果
    model_pred = []
    with torch.no_grad():
        for x, y, idx in predict_loader:
            ...
            # 计算损失值，并将每个样本的损失值存储到loss_outs数组中
            loss = crit(logits1, y)
            loss_outs[idx] = loss
